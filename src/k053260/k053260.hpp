	...
	private:
		...
		class voice_t : public vgsound_emu_core
		{
			public:
				...
				void set_busy(bool busy) { m_busy = busy ? 1 : 0; }

				void set_loop(bool loop) { m_loop = loop ? 1 : 0; }

				void set_adpcm(bool adpcm) { m_adpcm = adpcm ? 1 : 0; }

				void length_inc() { m_length = (m_length + 1) & 0xffff; }

				void set_pan(u8 pan) { m_pan = m_host.pan_dir[pan & 7]; }
				...
				u32 start() { return m_start; }

				u16 length() { return m_length; }

				s32 out(u8 ch) { return m_out[ch & 1]; }
		...
		class ctrl_t
		{
			public:
				...
				// getters
				bool rom_read() { return m_rom_read; }

				bool sound_en() { return m_sound_en; }

				u8 input_en() { return m_input_en; }
