...
class es5506_core : public es550x_shared_core
{
	private:
		class output_t
		{
			public:
				...
				void serial_in(bool ch, bool in)
				{
					if (ch)	 // Right output
					{
		...
		class voice_t : public es550x_voice_t
		{
			public:
				...
				// Setters
				void set_lvol(s32 lvol) { m_lvol = lvol; }

				void set_rvol(s32 rvol) { m_rvol = rvol; }

				void set_lvramp(s32 lvramp) { m_lvramp = lvramp; }

				void set_rvramp(s32 rvramp) { m_rvramp = rvramp; }

				void set_ecount(s16 ecount) { m_ecount = ecount; }

				// Getters
				s32 lvol() { return m_lvol; }

				s32 rvol() { return m_rvol; }

				s32 lvramp() { return m_lvramp; }

				s32 rvramp() { return m_rvramp; }

				s16 ecount() { return m_ecount; }

				filter_ramp_t &k2ramp() { return m_k2ramp; }

				filter_ramp_t &k1ramp() { return m_k1ramp; }
	...
	public:
		...
		s32 lout(u8 ch) { return m_output[std::min<u8>(5, ch & 0x7)].left(); }

		s32 rout(u8 ch) { return m_output[std::min<u8>(5, ch & 0x7)].right(); }
		...
		void set_mute(u8 ch, bool mute) { m_voice[ch & 0x1f].set_mute(mute); }

		// per-voice outputs
		s32 voice_lout(u8 voice) { return (voice < 32) ? m_voice[voice].left_out() : 0; }

		s32 voice_rout(u8 voice) { return (voice < 32) ? m_voice[voice].right_out() : 0; }
