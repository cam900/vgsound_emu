...
void k052539_core::write(u16 address, u8 data)
{
	...
	switch (bitfield(address, 11, 5))
	{
		...
		case 0b10111: // 0xb800-0xbfff SCC+, Mapper configuration
			...
			{
				m_mapper.ram_enable[0] = bitfield(data, 4) || bitfield(data, 0);
				m_mapper.ram_enable[1] = bitfield(data, 4) || bitfield(data, 1);
				m_mapper.ram_enable[2] = bitfield(data, 4) || bitfield(data, 2);
				m_mapper.ram_enable[3] = bitfield(data, 4);
				prev = m_is_sccplus;
	...
	if (update)
		m_scc_enable = m_is_sccplus ? bitfield(m_mapper.bank[3], 7) : (bitfield(m_mapper.bank[2], 0, 6) == 0x3f);
}
